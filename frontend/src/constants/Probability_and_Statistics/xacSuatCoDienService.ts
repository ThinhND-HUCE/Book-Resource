...
export const fetchExerciseQuestion = async (): Promise<ExerciseQuestion> => {
    const response = await fetch('http://localhost:8000/api/Probability_and_Statistics/xac_suat_co_dien/', {
...
export const submitExerciseAnswer = async (
    ...
    const response = await fetch('http://localhost:8000/api/Probability_and_Statistics/xac_suat_co_dien/submit/', {
        method: 'POST',
        headers: getAuthHeaders(),
        body: JSON.stringify({
            ...answer,
